...
def _get_ranked_sheet_schema(report_year: int) -> OrderedSchema:
    ...
    class RankedSheetSchema(OrderedSchema):
        ...
        # Contact info
        address = fields.String(attribute="address", allow_none=True, data_key="כתובת")
        email = fields.String(attribute="email", allow_none=True, data_key="אימייל")
        phone_numbers_str = fields.String(attribute="phone_numbers_str", allow_none=True, data_key="מספרי טלפון")
        ...
        __________ = fields.Str(dump_default=None)
...
def _get_appsheet_schema(report_year: int) -> OrderedSchema:

    class AppSheetSchema(OrderedSchema):
...
def _get_publish_sheet_values(
    general_info: pd.DataFrame, ranked_dfs: List[pd.DataFrame], sheet_type: str = "ranked"
) -> dict[int, list]:
    ...
    for idx, ranked_df in enumerate(ranked_dfs):
        ...
        if sheet_type == "ranked":
            sheet_schema = _get_ranked_sheet_schema(rank_year)
        elif sheet_type == "appsheet":
            sheet_schema = _get_appsheet_schema(rank_year)
        else:
...
def upload_spread_sheet(
    ...
    prefix = "ngo_ranking_"
    publish_sheet_values = _get_publish_sheet_values(general_info, ranked_dfs, sheet_type="ranked")
...
def upload_appsheet(general_info: pd.DataFrame, ranked_dfs: List[pd.DataFrame]) -> None:
    ...
    publish_sheet_values = _get_publish_sheet_values(general_info, ranked_dfs, sheet_type="appsheet")
